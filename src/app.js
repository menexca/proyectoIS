...
app.get('/Clientes/:Vendedor', async (req, res) => {
  ...
  const query = `SELECT CodigoCliente, Nombre, CONCAT(CAST((CodigoCliente) AS CHAR),' ',Nombre) as NombreBusqueda, (case when RazonSocial is null or RazonSocial='null' then '' else RazonSocial end) as RazonSocial, Direccion, (case when DiasVisita not in (1,2,3,4,5,6,7) then 8 else DiasVisita end) as DiasVisita, (case when Telefonos='null' then '' else Telefonos end) as Telefonos, IFNULL(LimiteCredito,0) as LimiteCredito, SaldoMonedaTotal, (case when SaldoMonedaVencido>SaldoMonedaTotal then SaldoMonedaTotal else SaldoMonedaVencido end) as SaldoMonedaVencido, date(ultimopago) as ultimopago, ProximoVencer, Estatus FROM Clientes WHERE Estatus<>'S' and Vendedor = ? Order by SaldoMonedaTotal desc`;
...
app.get('/ClientesSupervisor/:Vendedor', async (req, res) => {
  const vendedor = req.params.Vendedor;
  const query = `SELECT CodigoCliente, Nombre, CONCAT(CAST((CodigoCliente) AS CHAR),' ',Nombre) as NombreBusqueda, (case when RazonSocial is null or RazonSocial='null' then '' else RazonSocial end) as RazonSocial, Direccion, (case when DiasVisita not in (1,2,3,4,5,6,7) then 8 else DiasVisita end) as DiasVisita, (case when Telefonos='null' then '' else Telefonos end) as Telefonos, IFNULL(LimiteCredito,0) as LimiteCredito, SaldoMonedaTotal, (case when SaldoMonedaVencido>SaldoMonedaTotal then SaldoMonedaTotal else SaldoMonedaVencido end) as SaldoMonedaVencido, date(ultimopago) as ultimopago, ProximoVencer, Estatus FROM Clientes C WHERE Estatus<>'S' and EXISTS (SELECT * FROM Vendedores V WHERE V.Codigo=C.Vendedor AND V.SupervisadoPor = ? ) Order by SaldoMonedaTotal desc`;
...
app.get('/ClientesFechaSync/:Vendedor/:FechaSync', async (req, res) => {
  const vendedor = req.params.Vendedor;
  const fechaSync = req.params.FechaSync;
  const query = `SELECT CodigoCliente, Nombre, CONCAT(CAST((CodigoCliente) AS CHAR),' ',Nombre) as NombreBusqueda, (case when RazonSocial is null or RazonSocial='null' then '' else RazonSocial end) as RazonSocial, Direccion, (case when DiasVisita not in (1,2,3,4,5,6,7) then 8 else DiasVisita end) as DiasVisita, (case when Telefonos='null' then '' else Telefonos end) as Telefonos, IFNULL(LimiteCredito,0) as LimiteCredito, SaldoMonedaTotal, (case when SaldoMonedaVencido>SaldoMonedaTotal then SaldoMonedaTotal else SaldoMonedaVencido end) as SaldoMonedaVencido, date(ultimopago) as ultimopago, ProximoVencer, Estatus FROM Clientes WHERE Vendedor = ? and FechaSync >= ? Order by SaldoMonedaTotal desc`;
...
app.get('/ClientesSupervisorFechaSync/:Vendedor/:FechaSync', async (req, res) => {
  const vendedor = req.params.Vendedor;
  const fechaSync = req.params.FechaSync;
  const query = `SELECT CodigoCliente, Nombre, CONCAT(CAST((CodigoCliente) AS CHAR),' ',Nombre) as NombreBusqueda, (case when RazonSocial is null or RazonSocial='null' then '' else RazonSocial end) as RazonSocial, Direccion, (case when DiasVisita not in (1,2,3,4,5,6,7) then 8 else DiasVisita end) as DiasVisita, (case when Telefonos='null' then '' else Telefonos end) as Telefonos, IFNULL(LimiteCredito,0) as LimiteCredito, SaldoMonedaTotal, (case when SaldoMonedaVencido>SaldoMonedaTotal then SaldoMonedaTotal else SaldoMonedaVencido end) as SaldoMonedaVencido, date(ultimopago) as ultimopago, ProximoVencer, Estatus FROM Clientes C WHERE EXISTS (SELECT * FROM Vendedores V WHERE V.Codigo=C.Vendedor AND V.SupervisadoPor = ? ) and FechaSync >= ? Order by SaldoMonedaTotal desc`;
...
app.get('/ClientesFechaSync/:FechaSync', async (req, res) => {
  const fechaSync = req.params.FechaSync;
  const query = `SELECT CodigoCliente, Nombre, CONCAT(CAST((CodigoCliente) AS CHAR),' ',Nombre) as NombreBusqueda, (case when RazonSocial is null or RazonSocial='null' then '' else RazonSocial end) as RazonSocial, Direccion, (case when DiasVisita not in (1,2,3,4,5,6,7) then 8 else DiasVisita end) as DiasVisita, (case when Telefonos='null' then '' else Telefonos end) as Telefonos, IFNULL(LimiteCredito,0) as LimiteCredito, SaldoMonedaTotal, (case when SaldoMonedaVencido>SaldoMonedaTotal then SaldoMonedaTotal else SaldoMonedaVencido end) as SaldoMonedaVencido, date(ultimopago) as ultimopago, ProximoVencer, Estatus FROM Clientes WHERE FechaSync >= ? Order by SaldoMonedaTotal desc`;
...
app.get('/Pedidos/:Vendedor', async (req, res) => {
  const vendedor = req.params.Vendedor;
  const query = `SELECT P.Numero, P.FechaEmision, C.Nombre, CONCAT(CAST((P.Numero) AS CHAR),' ',C.Nombre) as NombreBusqueda, P.Vendedor, V.nombre as NombreVendedor, P.TotalBruto2, P.Impuesto2, P.TotalPedido2, P.Estatus, (SELECT count(CodigoProducto) FROM PedidosRenglones PR WHERE PR.Numero=P.Numero) AS Items, (SELECT SUM(ROUND(PR.Cantidad/P.CantidadxBulto,2)) FROM PedidosRenglones PR LEFT JOIN Productos P ON P.CodigoProducto=PR.CodigoProducto WHERE PR.Numero=P.Numero) as Bultos, Comentarios, C.RIF FROM Pedidos P LEFT JOIN Clientes C ON P.CodigoCliente=C.CodigoCliente LEFT JOIN Vendedores V ON P.Vendedor=V.codigo WHERE TotalPedido>0 and P.Vendedor = ? ORDER BY FechaEmision DESC`;
...
app.get('/PedidosFecha/:Fecha', async (req, res) => {
  ...
  const query = `SELECT P.Numero, P.FechaEmision, C.Nombre, CONCAT(CAST((P.Numero) AS CHAR),' ',C.Nombre) as NombreBusqueda, P.Vendedor, V.nombre as NombreVendedor, P.TotalBruto2, P.Impuesto2, P.TotalPedido2, P.Estatus, (SELECT count(CodigoProducto) FROM PedidosRenglones PR WHERE PR.Numero=P.Numero) AS Items, (SELECT SUM(ROUND(PR.Cantidad/P.CantidadxBulto,2)) FROM PedidosRenglones PR LEFT JOIN Productos P ON P.CodigoProducto=PR.CodigoProducto WHERE PR.Numero=P.Numero) as Bultos, P.CodigoCliente, P.Comentarios, IFNULL((select 100*SUM(Despacho)/SUM(Cantidad) from PedidosRenglones PR where PR.Numero=P.Numero),0) as PorcentajeFacturado, C.CodigoCliente as RIF FROM Pedidos P LEFT JOIN Clientes C ON P.CodigoCliente=C.CodigoCliente LEFT JOIN Vendedores V ON P.Vendedor=V.codigo WHERE TotalPedido>0 and FechaEmision >= ? ORDER BY FechaEmision DESC`;
...
app.get('/pedidos/PedidosVendedorFecha/:Vendedor/:Fecha', async (req, res) => {
  const vendedor = req.params.Vendedor;
  const fecha = req.params.Fecha;
  const query = `SELECT P.Numero, P.FechaEmision, C.Nombre, CONCAT(CAST((P.Numero) AS CHAR),' ',C.Nombre) as NombreBusqueda, P.Vendedor, V.nombre as NombreVendedor, P.TotalBruto2, P.Impuesto2, P.TotalPedido2, P.Estatus, (SELECT count(CodigoProducto) FROM PedidosRenglones PR WHERE PR.Numero=P.Numero) AS Items, (SELECT SUM(ROUND(PR.Cantidad/P.CantidadxBulto,2)) FROM PedidosRenglones PR LEFT JOIN Productos P ON P.CodigoProducto=PR.CodigoProducto WHERE PR.Numero=P.Numero) as Bultos, P.CodigoCliente, P.Comentarios, IFNULL((select 100*SUM(Despacho)/SUM(Cantidad) from PedidosRenglones PR where PR.Numero=P.Numero),0) as PorcentajeFacturado, C.CodigoCliente as RIF FROM Pedidos P LEFT JOIN Clientes C ON P.CodigoCliente=C.CodigoCliente LEFT JOIN Vendedores V ON P.Vendedor=V.codigo WHERE TotalPedido>0 and P.Vendedor = ? and FechaEmision >= ? ORDER BY FechaEmision DESC`;
...
app.get('/PedidosSupervisorFecha/:Supervisor/:Fecha', async (req, res) => {
  ...
  const fecha = req.params.Fecha;
  const query = `SELECT P.Numero, P.FechaEmision, C.Nombre, CONCAT(CAST((P.Numero) AS CHAR),' ',C.Nombre) as NombreBusqueda, P.Vendedor, V.nombre as NombreVendedor, P.TotalBruto2, P.Impuesto2, P.TotalPedido2, P.Estatus, (SELECT count(CodigoProducto) FROM PedidosRenglones PR WHERE PR.Numero=P.Numero) AS Items, (SELECT SUM(ROUND(PR.Cantidad/P.CantidadxBulto,2)) FROM PedidosRenglones PR LEFT JOIN Productos P ON P.CodigoProducto=PR.CodigoProducto WHERE PR.Numero=P.Numero) as Bultos, P.CodigoCliente, P.Comentarios, IFNULL((select 100*SUM(Despacho)/SUM(Cantidad) from PedidosRenglones PR where PR.Numero=P.Numero),0) as PorcentajeFacturado, C.CodigoCliente as RIF FROM Pedidos P LEFT JOIN Clientes C ON P.CodigoCliente=C.CodigoCliente LEFT JOIN Vendedores V ON P.Vendedor=V.codigo WHERE TotalPedido>0 and V.SupervisadoPor = ? and FechaEmision >= ? ORDER BY FechaEmision DESC`;
  
  try {
    const [rows] = await pool.query(query, [supervisor, fecha]);
    res.json(rows);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Error en la consulta' });
...
app.post('/PedidosRenglones', async (req, res) => {
  try {
    ...
    const valuesRenglones = [pedidoRenglon.numero, pedidoRenglon.almacen, pedidoRenglon.cantidad, pedidoRenglon.precioMoneda, pedidoRenglon.descuento, pedidoRenglon.totalRenglon, pedidoRenglon.indice, pedidoRenglon.tarifa, pedidoRenglon.precioMoneda, pedidoRenglon.totalRenglon, pedidoRenglon.codigoProducto];
...
app.get('/VendedoresDisponibles', async (req, res) => { 
  const [rows] = await pool.query(`SELECT codigo, nombre, Zona, Almacen, RTRIM(rif) as rif, RTRIM(nit) as nit, SupervisadoPor FROM Vendedores V WHERE NOT EXISTS (SELECT * FROM Usuarios U WHERE U.CodigoVendedor=V.codigo) ORDER BY nombre`)
  res.status(200).json(rows)
})
...
app.get('/Usuarios/:Usuario', async (req, res) => {
  try {
    ...
    const query = `SELECT Usuario, Contrasena, NombreCompleto, CorreoElectronico, FechaRegistro, IFNULL(UltimoInicioSesion,'2000-01-01 00:00:00') as UltimoInicioSesion, Rol, Estatus, CodigoVendedor, IFNULL(FechaNacimiento,'2000-01-01 00:00:00') as FechaNacimiento, Direccion, NumeroTelefono, Cedula, SupervisadoPor FROM Usuarios where Usuario = ?`;
